...
describe('Pomodoro class tests.', function () {
  ...
  it('Default constructor (55 min work, 5 min break, no warning).', function() {
    var p = new Pomodoro();
    expect(p.getPhase()).toBe(this._phase = Pomodoro.PHASE_WORK);
    expect(p.getTotalSeconds()).toBe(60 * 60);
    expect(p.getPhaseSeconds(Pomodoro.PHASE_WORK)).toBe(55 * 60);
    expect(p.getPhaseSeconds(Pomodoro.PHASE_BREAK)).toBe(5 * 60);
    expect(p.getPhaseSeconds(Pomodoro.PHASE_WARN)).toBe(0);
  });
  ...
  it('Constructor with specific time intervals.', function() {
    ...
    expect(p.getPhase()).toBe(this._phase = Pomodoro.PHASE_WORK);
    expect(p.getTotalSeconds()).toBe(60 * 60);
    expect(p.getPhaseSeconds(Pomodoro.PHASE_WORK)).toBe(work_m * 60);
    expect(p.getPhaseSeconds(Pomodoro.PHASE_BREAK)).toBe(break_m * 60);
    expect(p.getPhaseSeconds(Pomodoro.PHASE_WARN)).toBe(warm_m * 60);
  });
